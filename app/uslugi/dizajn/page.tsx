...
import { Palette, CheckCircle2, Clock, Shield, Phone, Eye } from 'lucide-react';
...
export default function DizajnPage() {
  ...
  const features = [
    'Авторський дизайн-проєкт',
    '3D-візуалізація кожної кімнати',
    'Підбір матеріалів та фурнітури',
    'Планування просторових рішень',
    'Специфікації для будівельників',
    'Контроль виконання проєкту',
  ];

  const process = [
    { num: '01', title: 'ЗУСТРІЧ ТА БРИФ', desc: 'Обговорення ваших бажань, потреб та бюджету' },
    { num: '02', title: 'ЗАМІРИ ТА АНАЛІЗ', desc: 'Детальні заміри об\'єкту, аналіз інсоляції та простору' },
    { num: '03', title: 'КОНЦЕПЦІЯ', desc: 'Створення концепції дизайну з ескізами та колірними рішеннями' },
    { num: '04', title: '3D-ВІЗУАЛІЗАЦІЯ', desc: 'Фотореалістичні рендери всіх приміщень' },
    { num: '05', title: 'РОБОЧА ДОКУМЕНТАЦІЯ', desc: 'Повний пакет креслень, специфікацій та планів' },
  ];
      ...
      <section ref={heroRef} className="relative h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        >
          ...
          <p className="text-white/60 text-lg md:text-xl font-sans max-w-2xl mx-auto mb-10">
            Авторські дизайн-проєкти з 3D-візуалізацією. Створимо простір вашої мрії.
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {styles.map((style, i) => (
              <motion.div
                ...
                className="p-6 border border-white/[0.06] hover:border-[#C9A84C]/30 text-center transition-all duration-300"
              >
                <Palette className="w-8 h-8 text-[#C9A84C] mx-auto mb-3" />
                <span className="text-white/80 font-heading text-xs uppercase tracking-wider">{style}</span>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: Eye, title: 'ФОТОРЕАЛІСТИЧНІ 3D', desc: 'Ви побачите кожну кімнату до початку робіт — як на фото' },
              { icon: Clock, title: 'ШВИДКИЙ СТАРТ', desc: 'Перші ескізи вже через 7 днів після замірів' },
              { icon: CheckCircle2, title: 'ПОВНИЙ ПАКЕТ', desc: 'Всі необхідні креслення та специфікації для реалізації' },
            ].map((benefit, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 30 }}
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/60 text-lg font-sans max-w-xl mx-auto mb-10">
                Залиште заявку, і наш дизайнер зв\'яжеться з вами для безкоштовної консультації
              </p>
