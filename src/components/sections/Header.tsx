...
export default function Header({ onOpenModal, onScrollTo }: HeaderProps) {
  ...
  return (
    <>
      ...
      >
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <button
            ...
            className="xl:hidden text-white hover:text-[#C9A84C] transition-colors relative z-[210]"
      ...
      {/* Mobile Menu — sibling of header so it is not trapped in the header's stacking context */}
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: '100%' }} transition={{ duration: 0.4, ease: THEME.EASE }}
            className="fixed inset-0 z-[200] bg-[#0e0e0e]/95 backdrop-blur-2xl flex flex-col p-8 pt-32 xl:hidden"
          >
            {/* Close button — always clickable, lives inside the overlay */}
            <button
              type="button"
            ...
            <div className="mt-auto flex flex-col gap-5 border-t border-white/10 pt-8">
              ...
              <button onClick={() => { onOpenModal(); setMobileOpen(false); }}
